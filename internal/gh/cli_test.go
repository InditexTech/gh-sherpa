...
func TestCli_CreateFork(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "Success - Single part fork name (organization only)",
			forkName:         "just-a-name",
			forkExistsError:  errors.New("Could not resolve to a Repository"),
			forkExistsResult: "",
			mockResult:       "✓ Created fork user/repo\n✓ Added remote \"origin\"",
			mockError:        nil,
			expectedArgs:     []string{"repo", "fork", "--remote", "--org", "just-a-name"},
			wantErr:          false,
		},
		{
			name:             "Success - Multiple parts (uses first as org)",
			forkName:         "org/repo/extra",
			forkExistsError:  errors.New("Could not resolve to a Repository"),
			forkExistsResult: "",
			mockResult:       "✓ Created fork user/repo\n✓ Added remote \"origin\"",
			mockError:        nil,
			expectedArgs:     []string{"repo", "fork", "--remote", "--org", "org"},
			wantErr:          false,
		},
		{
