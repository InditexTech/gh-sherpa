...
func init() {
	...
	Command.PersistentFlags().BoolVar(&flags.UseWorktree, "worktree", false, "create the branch in a new git worktree")
	Command.PersistentFlags().StringVar(&flags.WorktreePath, "worktree-path", "", "path for the new git worktree (implies --worktree)")
}
...
func runCommand(cmd *cobra.Command, _ []string) (err error) {
	...
	createBranchConfig := use_cases.CreateBranchConfiguration{
		...
		UseWorktree:     flags.UseWorktree || flags.WorktreePath != "",
